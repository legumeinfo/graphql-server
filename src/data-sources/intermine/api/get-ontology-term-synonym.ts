...
export async function getOntologyTermSynonym(name: string):
Promise<ApiResponse<GraphQLOntologyTermSynonym>> {
    const constraints = [intermineConstraint('OntologyTermSynonym.name', '=', name)];
    ...
    return this.pathQuery(query)
        ...
        .then((ontologyTermSynonym: GraphQLOntologyTermSynonym) => ({data: ontologyTermSynonym}));
}
