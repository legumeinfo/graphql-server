...
export async function getPanGenePairs(
  ...
  if (genus) {
    constraints.push(
      intermineConstraint('Gene.panGeneSets.genes.organism.genus', '=', genus),
    );
  }
  if (species) {
    constraints.push(
      intermineConstraint(
        'Gene.panGeneSets.genes.organism.species',
        '=',
        species,
      ),
    );
  ...
  if (strain) {
    constraints.push(
      intermineConstraint(
        'Gene.panGeneSets.genes.strain.identifier',
        '=',
        strain,
      ),
    );
  ...
  const dataPromise = this.pathQuery(query, {page, pageSize}).then(
    (response: InterminePanGenePairResponse) => response2panGenePairs(response),
  );
  ...
  const pageInfoPromise = allDataPromise.then(
    (response: InterminePanGenePairResponse) => {
      const fakeCountResponse: IntermineCountResponse = {
        count: response.results.length,
      };
      return countResponse2graphqlPageInfo(fakeCountResponse, page, pageSize);
    },
  );
  const resultsInfoPromise = allDataPromise.then(
    (response: InterminePanGenePairResponse) => {
      const reducer = (
      ...
      const fakeSummaryResponse: IntermineSummaryResponse = {
        uniqueValues,
        results,
      };
  ...
  return Promise.all([dataPromise, pageInfoPromise, resultsInfoPromise]).then(
    ([data, pageInfo, resultsInfo]) => ({
      data,
      metadata: {pageInfo, resultsInfo},
    }),
  );
}
