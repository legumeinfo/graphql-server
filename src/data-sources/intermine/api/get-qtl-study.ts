...
export async function getQTLStudy(
  identifier: string,
): Promise<ApiResponse<GraphQLQTLStudy>> {
  const constraints = [
    intermineConstraint('QTLStudy.primaryIdentifier', '=', identifier),
  ];
  ...
  return this.pathQuery(query)
    .then((response: IntermineQTLStudyResponse) =>
      response2qtlStudies(response),
    )
...
export async function getQTLStudyForTrait(
