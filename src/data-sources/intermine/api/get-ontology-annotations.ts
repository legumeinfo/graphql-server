...
async function getOntologyAnnotations(
  pathQuery: string,
  ...
  const dataPromise = this.pathQuery(pathQuery, {page, pageSize}).then(
    (response: IntermineOntologyAnnotationResponse) =>
      response2ontologyAnnotations(response),
  );
...
export async function getOntologyAnnotationsForAnnotatable(
  id: number,
  {page, pageSize}: PaginationOptions,
): Promise<ApiResponse<GraphQLOntologyAnnotation>> {
  const constraints = [
    intermineConstraint('OntologyAnnotation.subject.id', '=', id),
  ];
...
export async function getOntologyAnnotationsForOntologyTerm(
  id: number,
  {page, pageSize}: PaginationOptions,
): Promise<ApiResponse<GraphQLOntologyAnnotation>> {
  const constraints = [
    intermineConstraint('OntologyAnnotation.ontologyTerm.id', '=', id),
  ];
  const query = interminePathQuery(
    intermineOntologyAnnotationAttributes,
    intermineOntologyAnnotationSort,
