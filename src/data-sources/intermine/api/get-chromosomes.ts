...
export async function getChromosomes({
  ...
  page,
  pageSize,
}: GetChromosomesOptions): Promise<ApiResponse<GraphQLChromosome[]>> {
  const constraints = [];
  if (genus) {
    constraints.push(
      intermineConstraint('Chromosome.organism.genus', '=', genus),
    );
  }
  if (species) {
    constraints.push(
      intermineConstraint('Chromosome.organism.species', '=', species),
    );
  }
  if (strain) {
    constraints.push(
  ...
  if (assembly) {
    constraints.push(
      intermineConstraint('Gene.assemblyVersion', '=', assembly),
    );
  }
  if (annotation) {
    constraints.push(
  ...
  const dataPromise = this.pathQuery(query, {page, pageSize}).then(
    (response: IntermineChromosomeResponse) => response2chromosomes(response),
  );
