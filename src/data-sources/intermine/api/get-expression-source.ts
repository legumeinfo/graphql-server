...
export async function getExpressionSource(
  identifier: string,
): Promise<ApiResponse<GraphQLExpressionSource>> {
  const constraints = [
    intermineConstraint('ExpressionSource.primaryIdentifier', '=', identifier),
  ];
  ...
  return this.pathQuery(query)
    .then((response: IntermineExpressionSourceResponse) =>
      response2expressionSources(response),
    )
    .then((expressionSources: Array<GraphQLExpressionSource>) => {
      if (!expressionSources.length) return null;
      return expressionSources[0];
    })
    .then((expressionSource: GraphQLExpressionSource) => ({
      data: expressionSource,
    }));
}
