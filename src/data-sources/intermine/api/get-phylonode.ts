...
export async function getPhylonode(
  identifier: string,
): Promise<ApiResponse<GraphQLPhylonode>> {
  const constraints = [
    intermineConstraint('Phylonode.identifier', '=', identifier),
  ];
  ...
  return this.pathQuery(query)
    .then((response: InterminePhylonodeResponse) =>
      response2phylonodes(response),
    )
