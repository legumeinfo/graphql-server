...
export async function getPhylotree(
  identifier: string,
): Promise<ApiResponse<GraphQLPhylotree>> {
  const constraints = [
    intermineConstraint('Phylotree.primaryIdentifier', '=', identifier),
  ];
  ...
  return this.pathQuery(query)
    .then((response: InterminePhylotreeResponse) =>
      response2phylotrees(response),
    )
