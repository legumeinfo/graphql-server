...
async function getPublications(
  ...
  const dataPromise = this.pathQuery(pathQuery, {page, pageSize}).then(
    (response: InterminePublicationResponse) => response2publications(response),
  );
...
export async function getPublicationsForAnnotatable(
  id: number,
  {page, pageSize}: PaginationOptions,
): Promise<ApiResponse<GraphQLPublication>> {
...
export async function getPublicationsForAuthor(
  id: number,
  {page, pageSize}: PaginationOptions,
): Promise<ApiResponse<GraphQLPublication>> {
...
export async function getPublicationsForDataSource(
  id: number,
  {page, pageSize}: PaginationOptions,
): Promise<ApiResponse<GraphQLPublication>> {
