...
export async function getLinkageGroup(
  identifier: string,
): Promise<ApiResponse<GraphQLLinkageGroup>> {
  const constraints = [
    intermineConstraint('LinkageGroup.primaryIdentifier', '=', identifier),
  ];
  ...
  return this.pathQuery(query)
    .then((response: IntermineLinkageGroupResponse) =>
      response2linkageGroups(response),
    )
