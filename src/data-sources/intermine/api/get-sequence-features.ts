...
async function getSequenceFeatures(pathQuery: string): Promise<ApiResponse<GraphQLSequenceFeature>> {
    // get the data
    return this.pathQuery(pathQuery, {}, intermineSequenceFeatureQueryFormat)
...
export async function getChildFeaturesForSequenceFeature(id: number): Promise<ApiResponse<GraphQLSequenceFeature>> {
...
export async function getOverlappingFeaturesForSequenceFeature(id: number): Promise<ApiResponse<GraphQLSequenceFeature>> {
    const constraints = [intermineConstraint('SequenceFeature.id', '=', id)];
