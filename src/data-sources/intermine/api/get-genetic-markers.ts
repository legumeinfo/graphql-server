...
async function getGeneticMarkers(
  pathQuery: string,
  ...
  const dataPromise = this.pathQuery(pathQuery, {page, pageSize}).then(
    (response: IntermineGeneticMarkerResponse) =>
      response2geneticMarkers(response),
  );
...
export async function getGeneticMarkersForGenotypingPlatform(
  id: number,
  {page, pageSize}: PaginationOptions,
): Promise<ApiResponse<GraphQLGeneticMarker>> {
  const constraints = [
    intermineConstraint('GeneticMarker.genotypingPlatforms.id', '=', id),
  ];
...
export async function getGeneticMarkersForQTL(
  id: number,
  {page, pageSize}: PaginationOptions,
): Promise<ApiResponse<GraphQLGeneticMarker>> {
...
export async function getGeneticMarkersForGWASResult(
  id: number,
  {page, pageSize}: PaginationOptions,
): Promise<ApiResponse<GraphQLGeneticMarker>> {
  const constraints = [
    intermineConstraint('GeneticMarker.gwasResults.id', '=', id),
  ];
  const joins = sequenceFeatureJoinFactory('GeneticMarker');
  const query = interminePathQuery(
    intermineGeneticMarkerAttributes,
