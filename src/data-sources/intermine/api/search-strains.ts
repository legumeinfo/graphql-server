...
export async function searchStrains({
  ...
  page,
  pageSize,
}: SearchStrainsOptions): Promise<ApiResponse<GraphQLStrain[]>> {
  const constraints = [];
  if (description) {
    constraints.push(
      intermineConstraint('Strain.description', 'CONTAINS', description),
    );
  }
  if (origin) {
  ...
  if (species) {
    constraints.push(
      intermineConstraint('Strain.organism.species', '=', species),
    );
  }
  ...
  const dataPromise = this.pathQuery(query, {page, pageSize}).then(
    (response: IntermineStrainResponse) => response2strains(response),
  );
