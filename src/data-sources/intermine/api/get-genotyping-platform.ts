...
export async function getGenotypingPlatform(
  identifier: string,
  ...
  return this.pathQuery(query)
    .then((response: IntermineGenotypingPlatformResponse) =>
      response2genotypingPlatforms(response),
    )
    ...
    .then((genotypingPlatform: GraphQLGenotypingPlatform) => ({
      data: genotypingPlatform,
    }));
}
