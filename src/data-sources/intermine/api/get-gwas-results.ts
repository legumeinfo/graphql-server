...
async function getGWASResults(
  ...
  const dataPromise = this.pathQuery(query, {page, pageSize}).then(
    (response: IntermineGWASResultResponse) => response2gwasResults(response),
  );
...
export async function getGWASResultsForGWAS(
  id: number,
  {page, pageSize}: PaginationOptions,
): Promise<ApiResponse<GraphQLGWASResult>> {
...
export async function getGWASResultsForGeneticMarker(
  id: number,
  {page, pageSize}: PaginationOptions,
): Promise<ApiResponse<GraphQLGWASResult>> {
...
export async function getGWASResultsForTrait(
  id: number,
  {page, pageSize}: PaginationOptions,
): Promise<ApiResponse<GraphQLGWASResult>> {
