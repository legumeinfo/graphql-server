...
export async function getPanGeneSetsForGene(
  id: number,
  {page, pageSize}: PaginationOptions,
): Promise<ApiResponse<GraphQLPanGeneSet>> {
...
export async function getPanGeneSetsForTranscript(
  id: number,
  {page, pageSize}: PaginationOptions,
): Promise<ApiResponse<GraphQLPanGeneSet>> {
  const constraints = [
    intermineConstraint('PanGeneSet.transcripts.id', '=', id),
  ];
  const query = interminePathQuery(
    interminePanGeneSetAttributes,
    interminePanGeneSetSort,
...
export async function getPanGeneSetsForProtein(
  id: number,
  {page, pageSize}: PaginationOptions,
): Promise<ApiResponse<GraphQLPanGeneSet>> {
