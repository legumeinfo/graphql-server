...
async function getLocations(
  ...
  // get the data
  return this.pathQuery(pathQuery, {}, intermineLocationQueryFormat).then(
...
export async function getLocationsForBioEntity(
  identifier: string,
): Promise<ApiResponse<GraphQLLocation>> {
  const constraints = [
    intermineConstraint('Location.feature.primaryIdentifier', '=', identifier),
  ];
...
export async function getLocatedFeaturesForBioEntity(
  identifier: string,
): Promise<ApiResponse<GraphQLLocation>> {
  const constraints = [
    intermineConstraint(
