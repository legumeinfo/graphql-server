...
export async function getExpressionSample(
  identifier: string,
): Promise<ApiResponse<GraphQLExpressionSample>> {
  const constraints = [
    intermineConstraint('ExpressionSample.primaryIdentifier', '=', identifier),
  ];
  ...
  return this.pathQuery(query)
    .then((response: IntermineExpressionSampleResponse) =>
    ...
    .then((expressionSample: GraphQLExpressionSample) => ({
      data: expressionSample,
    }));
}
