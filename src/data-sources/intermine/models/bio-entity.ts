...
export const intermineBioEntityObjectAttributesFactory = (
  type = 'Annotatable',
) => [
...
export const intermineBioEntityObjectAttributes =
...
export type IntermineBioEntityResponse =
  IntermineDataResponse<IntermineBioEntityObject>;
// converts an Intermine jsonobjects response into an array of GraphQL BioEntity objects
export function response2bioEntities(
  response: IntermineBioEntityResponse,
): Array<GraphQLBioEntity> {
  const jsonResponse = objectsResponse2response(
    response,
    intermineBioEntityObjectAttributes,
  );
