...
export const intermineAnnotatableQueryFormat =
...
export const intermineAnnotatableObjectAttributesFactory = (
  type = 'Annotatable',
) => [`${type}.objectId`, `${type}.primaryIdentifier`];

export const intermineAnnotatableObjectAttributes =
  intermineAnnotatableObjectAttributesFactory();

export type IntermineAnnotatable = [number, string];

export const graphqlAnnotatableAttributes = ['id', 'identifier'];

export type IntermineAnnotatableResponse =
  IntermineDataResponse<IntermineAnnotatableObject>;
// converts an Intermine jsonobjects response into an array of GraphQL Annotation objects
export function response2annotations(
  response: IntermineAnnotatableResponse,
): Array<GraphQLAnnotatable> {
  const jsonResponse = objectsResponse2response(
    response,
    intermineAnnotatableObjectAttributes,
  );
