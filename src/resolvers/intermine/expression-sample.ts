...
export const expressionSampleFactory = (
  sourceName: KeyOfType<DataSources, IntermineAPI>,
): ResolverMap => ({
  Query: {
    expressionSample: async (_, {identifier}, {dataSources}) => {
      const {data: sample} =
        await dataSources[sourceName].getExpressionSample(identifier);
      if (sample == null) {
  ...
  ExpressionSample: {
    ...
    source: async (expressionSample, _, {dataSources}) => {
      return (
        dataSources[sourceName]
          .getExpressionSource(expressionSample.sourceIdentifier)
          // @ts-expect-error: implicit type any error
