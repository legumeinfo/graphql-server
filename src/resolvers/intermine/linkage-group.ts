...
export const linkageGroupFactory = (
  sourceName: KeyOfType<DataSources, IntermineAPI>,
): ResolverMap => ({
  Query: {
    linkageGroup: async (_, {identifier}, {dataSources}) => {
      const {data: group} =
        await dataSources[sourceName].getLinkageGroup(identifier);
      if (group == null) {
...
export const hasLinkageGroupFactory = (
  sourceName: KeyOfType<DataSources, IntermineAPI>,
): SubfieldResolverMap => ({
  linkageGroup: async (parent, _, {dataSources}, info) => {
    ...
    switch (typeName) {
      ...
      case 'QTL': {
        ...
        request = dataSources[sourceName].getLinkageGroup(
