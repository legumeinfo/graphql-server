...
export const chromosomeFactory = (
  sourceName: KeyOfType<DataSources, IntermineAPI>,
): ResolverMap => ({
  Query: {
    chromosome: async (_, {identifier}, {dataSources}) => {
      const {data: chromosome} =
        await dataSources[sourceName].getChromosome(identifier);
      if (chromosome == null) {
    ...
    chromosomes: async (
      _,
      {genus, species, strain, assembly, annotation, page, pageSize},
      {dataSources},
    ) => {
      const args = {
        genus,
