...
export const syntenyBlockFactory = (
  sourceName: KeyOfType<DataSources, IntermineAPI>,
): ResolverMap => ({
  Query: {
    syntenyBlock: async (_, {identifier}, {dataSources}) => {
      const {data: block} =
        await dataSources[sourceName].getSyntenyBlock(identifier);
      if (block == null) {
        const msg = `SyntenyBlock with identifier '${identifier}' not found`;
        inputError(msg);
      }
    ...
    syntenicRegions: async (syntenyBlock, _, {dataSources}) => {
      ...
      return (
        dataSources[sourceName]
          .getSyntenicRegionsForSyntenyBlock(id)
