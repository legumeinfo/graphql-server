...
export const phylotreeFactory = (
  sourceName: KeyOfType<DataSources, IntermineAPI>,
): ResolverMap => ({
  Query: {
    phylotree: async (_, {identifier}, {dataSources}) => {
      const {data: tree} =
        await dataSources[sourceName].getPhylotree(identifier);
      if (tree == null) {
  ...
  Phylotree: {
    ...
    newick: async (phylotree, _, {dataSources}) => {
      return (
        dataSources[sourceName]
          .getNewick(phylotree.identifier)
    ...
    nodes: async (phylotree, {page, pageSize}, {dataSources}) => {
      ...
      return (
        dataSources[sourceName]
          .getPhylonodesForPhylotree(id, {page, pageSize})
          // @ts-expect-error: implicit type any error
...
export const hasPhylotreeFactory = (
  sourceName: KeyOfType<DataSources, IntermineAPI>,
): SubfieldResolverMap => ({
  phylotree: async (parent, _, {dataSources}, info) => {
    ...
    switch (typeName) {
      ...
      case 'Phylonode': {
        ...
        return (
          dataSources[sourceName]
            .getPhylotree(phylotreeIdentifier)
            // @ts-expect-error: implicit type any error
            .then(({data: results}) => results)
        );
      }
