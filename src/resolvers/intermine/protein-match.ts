...
export const proteinMatchFactory = (
  sourceName: KeyOfType<DataSources, IntermineAPI>,
): ResolverMap => ({
  Query: {
    proteinMatch: async (_, {identifier}, {dataSources}) => {
      const {data: proteinMatch} =
        await dataSources[sourceName].getProteinMatch(identifier);
      if (proteinMatch == null) {
        const msg = `ProteinMatch with identifier '${identifier}' not found`;
        inputError(msg);
      }
