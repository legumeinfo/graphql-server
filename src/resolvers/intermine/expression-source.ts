...
export const expressionSourceFactory = (
  sourceName: KeyOfType<DataSources, IntermineAPI>,
): ResolverMap => ({
  Query: {
    expressionSource: async (_, {identifier}, {dataSources}) => {
      const {data: source} =
        await dataSources[sourceName].getExpressionSource(identifier);
      if (source == null) {
  ...
  ExpressionSource: {
    ...
    samples: async (expressionSource, {page, pageSize}, {dataSources}) => {
      ...
      return (
        dataSources[sourceName]
          .getExpressionSamplesForExpressionSource(id, args)
          // @ts-expect-error: implicit type any error
