...
export const isOntologyTermFactory = (
  ...
  crossReferences: async (ontologyTerm, {page, pageSize}, {dataSources}) => {
    ...
    return (
      dataSources[sourceName]
        .getCrossReferencesForOntologyTerm(id, {page, pageSize})
  ...
  parents: async (ontologyTerm, {page, pageSize}, {dataSources}) => {
    ...
    return (
      dataSources[sourceName]
        .getParentsForOntologyTerm(id, {page, pageSize})
        // @ts-expect-error: implicit type any error
        .then(({data: results}) => results)
    );
  },
  relations: async (ontologyTerm, _, {dataSources}) => {
    ...
    return (
      dataSources[sourceName]
        .getOntologyRelationsForOntologyTerm(id)
        // @ts-expect-error: implicit type any error
        .then(({data: results}) => results)
    );
  },
  synonyms: async (ontologyTerm, {page, pageSize}, {dataSources}) => {
    ...
    return (
      dataSources[sourceName]
        .getOntologyTermSynonymsForOntologyTerm(id, {page, pageSize})
        // @ts-expect-error: implicit type any error
        .then(({data: results}) => results)
    );
  },
