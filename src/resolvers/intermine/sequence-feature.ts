...
export const isSequenceFeatureFactory = (
  ...
  sequenceOntologyTerm: async (sequenceFeature, _, {dataSources}) => {
    return (
      dataSources[sourceName]
        .getSequenceOntologyTerm(sequenceFeature.soTermIdentifier)
  ...
  chromosome: async (sequenceFeature, _, {dataSources}) => {
    ...
    if (chromosomeIdentifier) {
      return (
        dataSources[sourceName]
          .getChromosome(sequenceFeature.chromosomeIdentifier)
          // @ts-expect-error: implicit type any error
          .then(({data: results}) => results)
      );
    }
  ...
  supercontig: async (sequenceFeature, _, {dataSources}) => {
    ...
    if (supercontigIdentifier) {
      return (
        dataSources[sourceName]
          .getSupercontig(sequenceFeature.supercontigIdentifier)
          // @ts-expect-error: implicit type any error
          .then(({data: results}) => results)
      );
    }
  ...
  chromosomeLocation: async (sequenceFeature, _, {dataSources}) => {
    ...
    if (chromosomeLocationId) {
      return (
        dataSources[sourceName]
          .getLocation(chromosomeLocationId)
          // @ts-expect-error: implicit type any error
          .then(({data: results}) => results)
      );
    }
  ...
  supercontigLocation: async (sequenceFeature, _, {dataSources}) => {
    ...
    if (supercontigLocationId) {
      return (
        dataSources[sourceName]
          .getLocation(supercontigLocationId)
          // @ts-expect-error: implicit type any error
          .then(({data: results}) => results)
      );
    }
  },
  overlappingFeatures: async (
    sequenceFeature,
    {page, pageSize},
    {dataSources},
  ) => {
    ...
    return (
      dataSources[sourceName]
        .getOverlappingFeaturesForSequenceFeature(id, {page, pageSize})
        // @ts-expect-error: implicit type any error
        .then(({data: results}) => results)
    );
  },
  childFeatures: async (sequenceFeature, {page, pageSize}, {dataSources}) => {
    ...
    return (
      dataSources[sourceName]
        .getChildFeaturesForSequenceFeature(id, {page, pageSize})
        // @ts-expect-error: implicit type any error
        .then(({data: results}) => results)
    );
  },
