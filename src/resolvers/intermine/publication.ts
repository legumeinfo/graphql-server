...
export const publicationFactory = (
  ...
  Query: {
    publication: async (_, {doi}, {dataSources}) => {
      const {data: publication} =
        await dataSources[sourceName].getPublication(doi);
      if (publication == null) {
  ...
  Publication: {
    authors: async (publication, {page, pageSize}, {dataSources}) => {
      ...
      return (
        dataSources[sourceName]
          .getAuthorsForPublication(id, {page, pageSize})
          // @ts-expect-error: implicit type any error
...
export const hasPublicationsFactory = (
  sourceName: KeyOfType<DataSources, IntermineAPI>,
): SubfieldResolverMap => ({
  publications: async (parent, {page, pageSize}, {dataSources}, info) => {
    ...
    switch (typeName) {
      case 'Author':
        ...
        break;
      case 'DataSource':
        request = dataSources[sourceName].getPublicationsForDataSource(
          id,
          args,
        );
        break;
