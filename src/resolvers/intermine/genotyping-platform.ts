...
export const genotypingPlatformFactory = (
  sourceName: KeyOfType<DataSources, IntermineAPI>,
): ResolverMap => ({
  Query: {
    genotypingPlatform: async (_, {identifier}, {dataSources}) => {
      const {data: genotypingPlatform} =
        await dataSources[sourceName].getGenotypingPlatform(identifier);
...
export const hasGenotypingPlatformFactory = (
  sourceName: KeyOfType<DataSources, IntermineAPI>,
): SubfieldResolverMap => ({
  genotypingPlatform: async (parent, _, {dataSources}, info) => {
    ...
    switch (typeName) {
      ...
      case 'GWAS': {
        ...
        request = dataSources[sourceName].getGenotypingPlatform(
